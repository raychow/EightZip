...
#define IMPLEMENT_IINARCHIVE_ADAPTER(target_name) \
    ...
    { \
        CHECK_OK(m_spTarget->Open(stream, &maxCheckStartPosition, openArchiveCallback) \
            , ArchiveException, "Cannot open the specified stream."); \
    } \
    void target_name##Adapter::Close() \
    { \
        CHECK_OK(m_spTarget->Close(), ArchiveException, "Cannot close the archive."); \
    } \
    ...
    { \
        UINT32 result = 0; \
        CHECK_OK(m_spTarget->GetNumberOfProperties(&result), ArchiveException, "Cannot get number of archive properties"); \
        return result; \
    } \
    void target_name##Adapter::GetPropertyInfo(UINT32 index, BSTR *name, PROPID *propID, VARTYPE *varType) \
    { \
        CHECK_OK(m_spTarget->GetPropertyInfo(index, name, propID, varType), ArchiveException, "Cannot get property info of the archive."); \
    } \
    UINT32 target_name##Adapter::GetNumberOfArchiveProperties() \
    { \
        UINT32 result = 0; \
        CHECK_OK(m_spTarget->GetNumberOfArchiveProperties(&result), ArchiveException, "Cannot get number of archive properties."); \
        return result; \
    } \
    void target_name##Adapter::GetArchivePropertyInfo(UINT32 index, BSTR *name, PROPID *propID, VARTYPE *varType) \
    { \
