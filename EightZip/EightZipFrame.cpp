...
void EightZipFrame::__Create()
{
    ...
    m_auiManager.AddPane(new FileExplorer(this), wxAuiPaneInfo().Name(wxT("FileExplorer")).CenterPane());
    m_auiManager.Update();
}
...
void EightZipFrame::__CreateToolBar()
{
    ...
    m_auiManager.AddPane(pToolBar, wxAuiPaneInfo().Caption("ToolBar").Name(wxT("ToolBar")).ToolbarPane().Top());
}
